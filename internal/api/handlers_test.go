...
func TestListRecipes_Default(t *testing.T) {
	...
	assert.Equal(t, float64(2), got[0]["servings"])
...
func TestGetRecipe_Success(t *testing.T) {
	...
	assert.Equal(t, float64(4), got["servings"])
	assert.Equal(t, float64(10), got["prep_minutes"])
	assert.Equal(t, float64(20), got["cook_minutes"])
	...
	require.True(t, ok)
	assert.Equal(t, float64(1), step["step_number"])
	...
	assert.Equal(t, "", ingredient["name"])
	assert.Equal(t, float64(400), ingredient["quantity"])
	assert.Equal(t, "g", ingredient["unit"])
	assert.Equal(t, false, ingredient["is_optional"])
	assert.Equal(t, "", ingredient["preparation_notes"])
